...
public:
	...
	vector<string> sub_OrbitsString;
	...
	Orbit_Obj *constructObject(string primaryRef) {
		primary = primaryRef;
	}

	void findSub_Orbits(multimap<string, string>& inputRef, Orbit_Obj& object) {
		for (const auto& ptr : inputRef) {
			if (ptr.first == object.primary) sub_OrbitsString.push_back(ptr.second);
		}
		for (const auto& ptr : sub_OrbitsString) {
			sub_Orbits.push_back(constructObject(ptr));
		}
	}
};
